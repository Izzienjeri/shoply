...
export default function AdminArtistDetailPage() {
  ...
  const artworkColumns: ColumnDef<ArtworkTypeFull>[] = useMemo(() => [
    ...
    },
    {
      ...
      header: "Artwork Name",
      cell: ({ row }) => (
        <Link href={`/artworks/${row.original.id}`} target="_blank" className="font-medium hover:underline hover:text-primary">
          {row.original.name} <ExternalLink className="inline h-3 w-3 ml-0.5" />
        </Link>
      ),
    },
    {
  ...
  if (error) {
    return (
      <div className="container mx-auto px-4 py-8">
        <Button variant="outline" size="sm" onClick={() => router.push('/admin/artists')} className="mb-6">
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8">
      <div className="flex justify-between items-center">
        <Button variant="outline" size="sm" onClick={() => router.push('/admin/artists')}>
          <ArrowLeft className="mr-2 h-4 w-4" /> Back to Artists List
        </Button>
      ...
      {artist.is_active === false && (
        <Alert variant="warning">
            ...
            <AlertTitle>This Artist is INACTIVE</AlertTitle>
        ...
        <CardHeader>
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6">
                ...
                <div className="flex-grow">
                    ...
                    <Badge variant={artist.is_active ? "default" : "destructive"}>
